...
int N;
long long C, arr [1000010], dp [1000010];
...
int main(){
    ...
    dp[1] = C; cht.add(-2*arr[1], arr[1]*arr[1]+dp[0]+C);
    for(int i = 2; i <= N; i++){
        dp[i] =  min(cht.get(arr[i])+arr[i]*arr[i], dp[i-1]+C);
        if(i == N) cout << dp[i] << '\n';
        else cht.add(-2*arr[i], arr[i]*arr[i]+dp[i-1]+C);
    }
    if(N == 1) cout << C << '\n';
    return 0;
}
